...
class MyBertModel(BertModel):
    ...
    def forward(
        ...
        """
        inputs_embeds = gnn_outputs

        if self.cls_embedding is not None:
            expand_cls_embedding = self.cls_embedding.expand(1, inputs_embeds.size(1), -1)
            inputs_embeds = torch.cat([expand_cls_embedding, inputs_embeds], dim=0)

...
class GATConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
                edge_attr: OptTensor, edge_index_new=None, weight=None, size: Size = None,
        ...
        if not self.expanded:
            ...
            if self.add_self_loops:
                if isinstance(edge_index_new, Tensor):
                    # We only want to add self-loops for nodes that appear both as
                    # source and target nodes:
                    num_nodes = x_src.size(0)
                    if x_dst is not None:
                        num_nodes = min(num_nodes, x_dst.size(0))
                    num_nodes = min(size) if size is not None else num_nodes
                    edge_index_new, weight = remove_self_loops(
                        edge_index_new, weight)
            ...
            out = self.propagate(edge_index_new, x=x, alpha=alpha, edge_attr=None,
...
#         return x_j * alpha.unsqueeze(-1)
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        # exit()
        if self.expanded:
            alpha = alpha + weight.reshape((-1,1))*self.rate
        if edge_attr is not None:
            if edge_attr.dim() == 1:
                edge_attr = edge_attr.view(-1, 1)
            assert self.lin_edge is not None

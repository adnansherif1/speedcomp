...
def floyd_warshall_fastest(adjacency_matrix):
    ...
    return mat
def floyd_warshall_fastest_torch(adjacency_matrix):
    mat = adjacency_matrix
    for k in range(len(mat)):
        # print(mat.unsqueeze(0)[k,:], mat.unsqueeze(0)[k,:].shape)
        # print(mat.unsqueeze(2)[:,k],mat.unsqueeze(2)[:,k].shape)
        mat = torch.min(mat, mat[k,:].unsqueeze(0) + mat.unsqueeze(2)[:,k])
    return mat
